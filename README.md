...
#### フロントエンド
- ReactとNext.jsで構築しました。言語にはTypeScriptを使用しています。
...
- Cookieには`httpOnly`属性を付与し、クライアント側でXSSが実行された場合にJavaScriptで取得されないように対応しています。
...
- 無限スクロールを実装する際に、次の取得の可否をサーバー側ではなくクライアント側で識別するような設計にしたせいで、クライアント側のコードが複雑になってしまいました。サーバーからのレスポンスの中にnextフラグのようなものを含めるべきだったと思います。
- 引用の取得は原則クライアントからのXHRで行なっているので、現状ページアクセスごとに通信が走る状態になっています。React Queryのようなライブラリを利用して、APIレスポンス結果をキャッシュすることで通信回数を抑えることができると考えています。
